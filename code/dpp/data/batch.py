...
class Batch(DotDict):
    ...
    """
    def __init__(self, inter_times: torch.Tensor, mask: torch.Tensor, marks: Optional[torch.Tensor] = None, interval: Optional[torch.Tensor] =None, **kwargs):
    ...
    def from_list(sequences: List[Sequence]):
        ...
        for i, seq in enumerate(sequences):
            interval.append(seq['interval'])

        interval = torch.tensor(interval)

        return Batch(inter_times, mask, marks, interval)
