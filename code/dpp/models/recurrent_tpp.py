...
class RecurrentTPP(nn.Module):
    ...
    def log_prob(self, batch: dpp.data.Batch) -> torch.Tensor:
        ...
        multi_marks = one_hot(batch.marks, num_classes=self.num_marks).float() # (batch_size, seq_len, num_marks)
        ...
        log_p = inter_time_dist.log_prob(inter_times)  # (batch_size, seq_len, num_marks)
       
        pos_log_p = log_p * multi_marks # (batch_size, seq_len, num_marks)
        neg_log_p = log_p * (1 - multi_marks)
        ...
        last_event_idx = last_event_idx.squeeze()  #(batch_size,)
        last_event_idx = last_event_idx.view(-1, 1, 1).repeat(1, 1, self.num_marks) #(batch_size, 1, num_marks)
        log_surv_all = inter_time_dist.log_survival_function(inter_times)  #(batch_size, seq_len, num_marks)
        log_surv_last = torch.gather(log_surv_all, 1, last_event_idx).squeeze() #(batch_size, num_marks)

        last_mark_idx = batch.mask.sum(-1, keepdim=True).long() - 1
        last_mark_idx = last_mark_idx.squeeze()  #(batch_size,)
        last_mark_idx = last_mark_idx.view(-1, 1, 1).repeat(1, 1, self.num_marks)
        last_mark = torch.gather(multi_marks, 1, last_mark_idx).squeeze() #(batch_size, num_marks)
        log_surv_last = (log_surv_last * last_mark).sum(-1) #(batch_size,)

        mark_logits = torch.log_softmax(self.mark_linear(context), dim=-1)  # (batch_size, seq_len, num_marks)
        tot_nll = (pos_log_p + mark_logits * multi_marks).sum(-1) * batch.mask #(batch_size, seq_len)
        tot_nll = tot_nll.sum(-1) + log_surv_last #(batch_size)

        mark_dist = Categorical(logits=mark_logits)
        log_mark = mark_dist.log_prob(batch.marks)
        log_mark = log_mark * batch.mask #(batch_size, seq_len)

        mark_class_joint = ((log_p + mark_logits).argmax(-1) * batch.mask).float() #(batch_size, seq_len)
        log_time = pos_log_p.sum(-1) * batch.mask

        return tot_nll, log_time, log_mark, log_surv_last, mark_class_joint  
    ...
    def sample(self, t_end: float, batch_size: int = 1, context_init: torch.Tensor = None) -> dpp.data.Batch:
        ...
        generated = False
        while not generated:
            inter_time_dist = self.get_inter_time_dist(next_context) # (batch_size, 1, num_marks)   
            next_inter_times = inter_time_dist.sample()  # (batch_size, 1, num_marks)   
            ...
            inter_times = torch.cat([inter_times, next_inter_time], dim=-1)  # (batch_size, seq_len)
